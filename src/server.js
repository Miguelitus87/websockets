//unicamente websockect requerimos el modulo nativo de http
//para todo lo demas http
...
socketServer.on('connection', (client) => {
    ...
    client.on("mensaje", (mensaje) => {
        getMessages().then((data) => {
            data.push(mensaje)
            fs.promises.writeFile('./src/msjs.txt', JSON.stringify(data));
...
async function getMessages() {
    try {
        const response = await fs.promises.readFile('./src/msjs.txt', 'utf-8');
        return JSON.parse(response);
    } catch (e) {
        return { error: true }
    }
